import pytest
from testdata.person_mocks import person_film_mock, person_mock, response_person_film_mock, response_person_mock
...
@pytest.mark.parametrize(
    "preload_data, person_id, expected_answer",
    [
        [
            {
...
async def test__get_person_details(
    clear_cache, es_write_data, make_get_request, preload_data, person_id, expected_answer
):
...
@pytest.mark.asyncio()
async def test__get_person_details_cache(es_write_data, make_get_request, preload_data, person_id, expected_answer):
    for name, data in preload_data.items():
        data_to_load = [{"_index": name, "_id": row["id"], "_source": row} for row in data]
        await es_write_data(name, data_to_load)
...
@pytest.mark.asyncio()
async def test__get_person_films_cache(es_write_data, make_get_request, preload_data, person_id, expected_answer):
    for name, data in preload_data.items():
        data_to_load = [{"_index": name, "_id": row["id"], "_source": row} for row in data]
        await es_write_data(name, data_to_load)
