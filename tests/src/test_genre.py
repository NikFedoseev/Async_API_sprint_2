...
@pytest.mark.parametrize(
    "preload_data, genre_id, expected_answer",
    [
        [
            {
        ...
        [
            ...
            {"status": 404, "data": {"detail": "Genre not found"}},
        ],
        [
            {
                ESIndex.genres.value: [
...
@pytest.mark.asyncio()
async def test__get_genre_by_id(clear_cache, es_write_data, make_get_request, preload_data, genre_id, expected_answer):
...
@pytest.mark.asyncio()
async def test__get_genre_by_id_cache(es_write_data, make_get_request, preload_data, genre_id, expected_answer):
    for name, data in preload_data.items():
        data_to_load = [{"_index": name, "_id": row["id"], "_source": row} for row in data]
        await es_write_data(name, data_to_load)
...
@pytest.mark.asyncio()
async def test__get_genres(clear_cache, es_write_data, make_get_request, preload_data, expected_answer):
    for name, data in preload_data.items():
        data_to_load = [{"_index": name, "_id": row["id"], "_source": row} for row in data]
        await es_write_data(name, data_to_load)
...
@pytest.mark.asyncio()
async def test__get_genres_cache(es_write_data, make_get_request, preload_data, expected_answer):
    for name, data in preload_data.items():
        data_to_load = [{"_index": name, "_id": row["id"], "_source": row} for row in data]
        await es_write_data(name, data_to_load)
